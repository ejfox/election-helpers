...
export function getGeneralElectionDate(year) {
  ...
  if (typeof year !== 'number') {
    throw new Error(
      `Expected number, got ${typeof year}. Try: getGeneralElectionDate(2024)`
    );
  }
  ...
  if (!isFinite(year)) {
    throw new Error(
      `Year must be finite, got ${year}. Try: getGeneralElectionDate(2024)`
    );
  }

  if (!Number.isInteger(year)) {
    throw new Error(
      `Year must be whole number, got ${year}. Try: getGeneralElectionDate(2024)`
    );
  }
  ...
  if (year < 1845) {
    throw new Error(
      `Year ${year} too early. US elections standardized in 1845. Try: getGeneralElectionDate(1845)`
    );
  }

  if (year > 9999) {
    throw new Error(
      `Year ${year} too far ahead. Maximum year: 9999. Try: getGeneralElectionDate(2024)`
    );
  }
  ...
  if (typeof date === 'string') {
    ...
    if (dateMatch) {
      ...
      if (parsedMonth < 1 || parsedMonth > 12) {
        throw new Error(
          `Invalid month ${parsedMonth} in "${date}". Try: isGeneralElectionDay("2024-11-05")`
        );
      }
      if (parsedDay < 1 || parsedDay > 31) {
        throw new Error(
          `Invalid day ${parsedDay} in "${date}". Try: isGeneralElectionDay("2024-11-05")`
        );
      }
    ...
    if (isNaN(checkDate.getTime())) {
      throw new Error(
        `Invalid date "${date}". Try: isGeneralElectionDay("2024-11-05")`
      );
    }
  } else if (date instanceof Date) {
    if (isNaN(date.getTime())) {
      throw new Error(
        `Invalid Date object. Try: isGeneralElectionDay("2024-11-05")`
      );
    }
    checkDate = date;
  } else {
    throw new Error(
      `Expected string or Date, got ${typeof date}. Try: isGeneralElectionDay("2024-11-05")`
    );
  }
...
export function getNextElectionDate(fromDate = new Date()) {
  ...
  if (typeof fromDate === 'string') {
    ...
    if (isNaN(startDate.getTime())) {
      throw new Error(
        `Invalid date "${fromDate}". Try: getNextElectionDate("2024-01-01")`
      );
    }
  } else if (fromDate instanceof Date) {
    if (isNaN(fromDate.getTime())) {
      throw new Error(
        `Invalid Date object. Try: getNextElectionDate("2024-01-01")`
      );
    }
    startDate = fromDate;
  } else {
    throw new Error(
      `Expected string or Date, got ${typeof fromDate}. Try: getNextElectionDate("2024-01-01")`
    );
  }
...
export function isPresidentialElectionYear(year) {
  ...
  if (typeof year !== 'number') {
    throw new Error(
      `Expected number, got ${typeof year}. Try: isPresidentialElectionYear(2024)`
    );
  }
  ...
  if (isNaN(year)) {
    throw new Error(
      `Year cannot be NaN. Try: isPresidentialElectionYear(2024)`
    );
  }

  if (!isFinite(year)) {
    throw new Error(
      `Year must be finite, got ${year}. Try: isPresidentialElectionYear(2024)`
    );
  }

  if (!Number.isInteger(year)) {
    throw new Error(
      `Year must be whole number, got ${year}. Try: isPresidentialElectionYear(2024)`
    );
  }
  ...
  if (year < 1792) {
    throw new Error(
      `Presidential elections started in 1792, got ${year}. Try: isPresidentialElectionYear(1792)`
    );
  }

  if (year > 9999) {
    throw new Error(
      `Year ${year} too far ahead. Maximum year: 9999. Try: isPresidentialElectionYear(2024)`
    );
  }
...
export function isMidtermElectionYear(year) {
  ...
  if (typeof year !== 'number') {
    throw new Error(
      `Expected number, got ${typeof year}. Try: isMidtermElectionYear(2026)`
    );
  }
  ...
  if (!isFinite(year)) {
    throw new Error(
      `Year must be finite, got ${year}. Try: isMidtermElectionYear(2026)`
    );
  }

  if (!Number.isInteger(year)) {
    throw new Error(
      `Year must be whole number, got ${year}. Try: isMidtermElectionYear(2026)`
    );
  }
  ...
  if (year < 1794) {
    throw new Error(
      `Midterm elections started in 1794, got ${year}. Try: isMidtermElectionYear(1794)`
    );
  }

  if (year > 9999) {
    throw new Error(
      `Year ${year} too far ahead. Maximum year: 9999. Try: isMidtermElectionYear(2026)`
    );
  }
...
export function getPrimaryDate(stateAbbr, year, party = 'both') {
  ...
  if (typeof stateAbbr !== 'string') {
    throw new Error(
      `Expected string for state, got ${typeof stateAbbr}. Try: getPrimaryDate("IA", 2024)`
    );
  }

  if (!stateAbbr || stateAbbr.trim().length === 0) {
    throw new Error(
      `State abbreviation required. Try: getPrimaryDate("IA", 2024)`
    );
  }
  ...
  if (cleanAbbr.length !== 2) {
    throw new Error(
      `State abbreviation must be 2 letters, got "${stateAbbr}". Try: getPrimaryDate("IA", 2024)`
    );
  }
  ...
  if (typeof year !== 'number') {
    throw new Error(
      `Expected number for year, got ${typeof year}. Try: getPrimaryDate("IA", 2024)`
    );
  }

  if (isNaN(year) || !isFinite(year) || !Number.isInteger(year)) {
    throw new Error(
      `Year must be valid integer, got ${year}. Try: getPrimaryDate("IA", 2024)`
    );
  }

  if (year < 1845 || year > 9999) {
    throw new Error(
      `Year ${year} out of range (1845-9999). Try: getPrimaryDate("IA", 2024)`
    );
  }
  ...
  if (typeof party !== 'string') {
    throw new Error(
      `Expected string for party, got ${typeof party}. Try: getPrimaryDate("IA", 2024, "D")`
    );
  }
  ...
  if (!validParties.includes(party)) {
    throw new Error(
      `Party must be "D", "R", or "both", got "${party}". Try: getPrimaryDate("IA", 2024, "D")`
    );
  }
...
export function getElectionDatesForYear(year) {
  ...
  if (typeof year !== 'number') {
    throw new Error(
      `Expected number, got ${typeof year}. Try: getElectionDatesForYear(2024)`
    );
  }
  ...
  if (!isFinite(year)) {
    throw new Error(
      `Year must be finite, got ${year}. Try: getElectionDatesForYear(2024)`
    );
  }

  if (!Number.isInteger(year)) {
    throw new Error(
      `Year must be whole number, got ${year}. Try: getElectionDatesForYear(2024)`
    );
  }
  ...
  if (year < 1845) {
    throw new Error(
      `Year ${year} too early. US elections standardized in 1845. Try: getElectionDatesForYear(1845)`
    );
  }

  if (year > 9999) {
    throw new Error(
      `Year ${year} too far ahead. Maximum year: 9999. Try: getElectionDatesForYear(2024)`
    );
  }
...
export function getDaysUntilElection(fromDate = new Date()) {
  ...
  if (typeof fromDate === 'string') {
    ...
    if (isNaN(startDate.getTime())) {
      throw new Error(
        `Invalid date "${fromDate}". Try: getDaysUntilElection("2024-01-01")`
      );
    }
  } else if (fromDate instanceof Date) {
    if (isNaN(fromDate.getTime())) {
      throw new Error(
        `Invalid Date object. Try: getDaysUntilElection("2024-01-01")`
      );
    }
    startDate = fromDate;
  } else {
    throw new Error(
      `Expected string or Date, got ${typeof fromDate}. Try: getDaysUntilElection("2024-01-01")`
    );
  }
...
export function formatElectionDate(date, format = 'long') {
  ...
  if (!(date instanceof Date)) {
    throw new Error(
      `Expected Date object, got ${typeof date}. Try: formatElectionDate(new Date())`
    );
  }
  ...
  if (typeof format !== 'string') {
    throw new Error(
      `Expected string for format, got ${typeof format}. Try: formatElectionDate(date, "long")`
    );
  }
  ...
  if (!validFormats.includes(format)) {
    throw new Error(
      `Format must be "long", "short", or "iso", got "${format}". Try: formatElectionDate(date, "long")`
    );
  }
