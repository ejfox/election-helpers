...
export function normalizeParty(party, customMap = {}) {
  ...
  if (typeof party !== 'string') {
    throw new Error(`Expected string for party, got ${typeof party}. Try: normalizeParty("Republican")`);
  }
  
  // Validate custom map
  if (customMap && typeof customMap !== 'object') {
    throw new Error(`Custom map must be object, got ${typeof customMap}. Try: normalizeParty("Rep", {"rep": "R"})`);
  }
...
export function isThirdParty(normalizedParty) {
  ...
  const major = ['R', 'D'];
  return !major.includes(normalizedParty) && !nonPartisan.includes(normalizedParty);
}